 ...
 * @version		2.2.0
 * @date		2014-06-23
 * @UpdateDate  2014-11-01 [Jumpsea]
 *
 */
 
require_once( FB_PATH.'/Entities/SignedRequest.php');
require_once( FB_PATH.'/Entities/AccessToken.php');
require_once( FB_PATH.'/HttpClients/FacebookCurl.php');
require_once( FB_PATH.'/HttpClients/FacebookHttpable.php');
require_once( FB_PATH.'/HttpClients/FacebookCurlHttpClient.php');
require_once( FB_PATH.'/HttpClients/FacebookStreamHttpClient.php');
require_once( FB_PATH.'/FacebookSession.php' );
require_once( FB_PATH.'/FacebookRequest.php' );
require_once( FB_PATH.'/FacebookResponse.php' );
require_once( FB_PATH.'/FacebookSDKException.php' );
require_once( FB_PATH.'/FacebookRequestException.php' );
require_once( FB_PATH.'/FacebookSignedRequestFromInputHelper.php');
require_once( FB_PATH.'/FacebookAuthorizationException.php' );
require_once( FB_PATH.'/FacebookRedirectLoginHelper.php' );
require_once( FB_PATH.'/FacebookCanvasLoginHelper.php');
require_once( FB_PATH.'/GraphObject.php' );
require_once( FB_PATH.'/GraphSessionInfo.php' );
...
class Facebook_v3 {
	...
	***/
	...
	{	
		...
		if (isset($_GET['redirect']) && !empty($_GET['redirect'])){
			echo "<script type='text/javascript'>parent.location.href = '{$_GET['redirect']}';</script>"; exit;
		}
		else if (isset($_GET['re-direct']) && !empty($_GET['re-direct']))
		{
			echo "<script type='text/javascript'>parent.location.href = '{$_GET['re-direct']}';</script>"; exit;
		}
	}
	...
	***/
	public function _init($param=''){
		...
		if ((!empty($error) && !empty($error_code)) OR ($this->SessionFailedCount() >= 2))
		{
	...
	***/
	...
	}//end public function facebookredirectloginhelper
